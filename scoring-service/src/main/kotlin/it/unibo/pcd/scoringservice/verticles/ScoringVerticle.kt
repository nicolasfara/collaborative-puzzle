...
class ScoringVerticle : CoroutineVerticle() {
    ...
    override suspend fun start() {
        ...
        client.basicConsumerAwait(SCORE_QUEUE).handler {
            logger.info("[Scoring-Service]-Receive message from Puzzle Manger")
            ...
            launch {
                ...
                logger.info("[Scoring-Service]-Store new score")
            }
        }
        client.basicConsumerAwait(SCORE_STATISTIC_QUEUE).handler{
            logger.info("[Scoring-Service]-Receive message from Puzzle service")
        }
    }
}
